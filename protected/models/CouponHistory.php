...
class CouponHistory extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('coupon_id, order_id, total_amount, session_id, date, user_id, status', 'required'),
			array('order_id, total_amount, user_id, status', 'numerical', 'integerOnly'=>true),
			array('session_id', 'numerical'),
			array('coupon_id', 'length', 'max'=>225),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, coupon_id, order_id, total_amount, session_id, date, user_id, status', 'safe', 'on'=>'search'),
		);
	}
